...
| Suite | File | Coverage |
|-------|------|----------|
| Home route | `src/routes/home.route.test.ts` | GET `/`, document shell, viewport meta, 404 for unknown paths |
| Styles route | `src/routes/styles.route.test.ts` | GET `/styles/layout.css`, responsive CSS rules, missing stylesheet 404 |
...
2. **GET `/` — browser check** — Opening the root URL on a narrow and wide viewport renders a readable page without horizontal scrolling.
...
| Responsive UI | Viewport meta in HTML; `layout.css` includes mobile-first media queries (covered by `npm test`) |
